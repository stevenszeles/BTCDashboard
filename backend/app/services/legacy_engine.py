...
def _get_benchmark() -> str:
    return (_get_setting("benchmark", DEFAULT_BENCH) or DEFAULT_BENCH).strip().upper()
...
def get_bench_series(bench_symbol: Optional[str] = None) -> pd.DataFrame:
    bench = (bench_symbol or _get_benchmark()).strip().upper()
    ...
    def _run(conn):
        ...
        cur.execute(
            ...
            SELECT p.account, p.instrument_id, p.qty, p.price, p.market_value, p.avg_cost, p.sector,
                   p.owner, p.strategy, p.strategy_id, p.strategy_name,
    ...
    if baseline_qty:
        for sym, qty in baseline_qty.items():
            ...
            if sym not in qty_piv.columns:
                qty_piv[sym] = 0.0
            qty_piv[sym] = qty_piv[sym].astype(float) + float(qty)
...
def _get_nav_series_from_snapshots(limit: int = 120, account: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if account_value is not None and (not rows or rows[-1].get("date") != today):
        rows = rows + [{"date": today, "nav": float(account_value), "bench": float(account_value)}]

    points = []
    for row in rows[-int(limit) :]:
        d = str(row.get("date") or "")
        nav_val = float(row.get("nav") or 0.0)
        bench_val = row.get("bench")
        if bench_val is None and bench_map:
            bench_val = bench_map.get(d)
        bench_val = nav_val if bench_val is None else float(bench_val)
        points.append({"date": d, "nav": nav_val, "bench": bench_val, "twr": None})
    return points
...
def _upsert_position_row(conn, account: str, instrument_id: str, qty: float, price: float, avg_cost: float, multiplier: float, sector: Optional[str], strategy_id: Optional[str], strategy_name: Optional[str]) -> None:
    cur = conn.cursor()
    cur.execute(
        """
        INSERT OR REPLACE INTO positions(
            account, instrument_id, qty, price, market_value, avg_cost, sector, strategy, strategy_id, strategy_name
        ) VALUES(?,?,?,?,?,?,?,?,?,?)
        """,
        (
            account,
...
def upsert_position(data: Dict[str, Any]) -> None:
    ...
    strategy_name = data.get("strategy_name") or strategy
    ...
    def _run(conn):
        ...
        _upsert_position_row(conn, account, instrument_id, qty, price, avg_cost, multiplier, sector, strategy_id, strategy_name)
        conn.commit()
...
def store_nav_snapshot(account: Optional[str], date_str: str, nav: float, bench: Optional[float] = None) -> None:
    def _run(conn):
        ...
        cur.execute(
            "INSERT OR REPLACE INTO nav_snapshots(account, date, nav, bench) VALUES(?,?,?,?)",
            (_account_label(account), date_str, float(nav), float(bench or nav)),
        )
        conn.commit()
    with_conn(_run)
    _clear_nav_cache()
        ...
        while not STOP_BACKGROUND_UPDATES:
            try:
                ...
                if pos is not None and not pos.empty:
                    symbols.update(pos["symbol"].astype(str).str.upper().tolist())
                bench = _get_benchmark()
                if bench:
                    symbols.add(bench)
                ...
                for sym in symbols:
                    if STOP_BACKGROUND_UPDATES:
                        break
                    try:
                        fetch_prices_incremental(sym, lookback, is_bench=(sym == bench))
                    except Exception:
                        continue
                time.sleep(60)
            except Exception:
